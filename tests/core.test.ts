...
function repository(): string {
  ...
  writeFileSync(join(root, "package.json"), JSON.stringify({ name: "fixture", scripts: { lint: "oxlint .", typecheck: "tsc --noEmit", "test:unit": "bun test", build: "vite build" }, devDependencies: { react: "1", vite: "1" } }));
...
describe("coding-tooling plans", () => {
  ...
  test("creates a deterministic fast plan and reports missing capabilities", () => {
    ...
    expect(plan.checks.map((check) => check.capability)).toEqual(["lint", "typecheck", "test:unit", "build"]);
