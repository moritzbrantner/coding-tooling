...
function readPullRequest(runner: Runner, root: string, prNumber: number): {
...
function restoreCheckout(
  ...
  if (checkout.status !== 0)
    diagnostics.push(commandDiagnostic("restore-checkout-failed", "Could not restore checkout", checkout));
  const removeRef = runner("git", ["update-ref", "-d", temporaryRef], root);
  if (removeRef.status !== 0)
    diagnostics.push(commandDiagnostic("temporary-ref-cleanup-failed", "Could not remove temporary PR ref", removeRef));
  return diagnostics;
}
...
export function integratePullRequest(
    ...
    return envelope("unavailable", started, data, [
      {
        ...
        message: "PR integration requires a clean working tree so the original checkout can be restored safely",
      },
    ]);
  ...
  if (fetchBase.status !== 0)
    return envelope("unavailable", started, data, [
      commandDiagnostic("base-fetch-failed", `Could not fetch ${remote}/${pr.baseRefName}`, fetchBase),
    ]);
  ...
  if (baseShaResult.status !== 0 || headShaResult.status !== 0) {
    ...
    return envelope("error", started, data, [
      ...(baseShaResult.status !== 0
        ? [commandDiagnostic("base-revision-failed", "Could not resolve fetched base", baseShaResult)]
        : []),
      ...(headShaResult.status !== 0
        ? [commandDiagnostic("pr-revision-failed", "Could not resolve fetched PR head", headShaResult)]
        : []),
    ]);
  }
  ...
  if (syntheticMerge.status !== 0) {
    const restoreDiagnostics = restoreCheckout(runner, root, originalBranch, originalHead, temporaryRef);
  ...
  if (postPipelineStatus.status !== 0 || postPipelineStatus.stdout.trim()) {
    ...
    return envelope("failed", started, data, [
      postPipelineStatus.status !== 0
        ? commandDiagnostic("post-pipeline-status-failed", "Could not inspect pipeline mutations", postPipelineStatus)
        : {
            code: "pipeline-mutated-tracked-files",
            message: "The local pipeline changed tracked files; integration stopped instead of merging an untested tree",
          },
      ...restoreDiagnostics,
    ]);
  }
  ...
  if (!current.info) {
    const restoreDiagnostics = restoreCheckout(runner, root, originalBranch, originalHead, temporaryRef);
    return envelope("unavailable", started, data, [
      commandDiagnostic("pr-refresh-failed", "Could not refresh pull request metadata", current.command),
      ...restoreDiagnostics,
    ]);
  }
  ...
  const currentBaseSha = currentBaseShaResult.status === 0 ? currentBaseShaResult.stdout.trim() : "";
  ...
  if (refreshBase.status !== 0 || currentBaseShaResult.status !== 0)
    blockingDiagnostics.push(
      refreshBase.status !== 0
        ? commandDiagnostic("base-refresh-failed", "Could not refresh the target branch", refreshBase)
        : commandDiagnostic("base-revision-failed", "Could not resolve refreshed target branch", currentBaseShaResult),
    );
  ...
  if (refreshed.reviewDecision === "CHANGES_REQUESTED" || refreshed.reviewDecision === "REVIEW_REQUIRED")
    blockingDiagnostics.push({
  ...
  if (merge.status !== 0)
    return envelope("failed", started, data, [
      commandDiagnostic("pr-merge-failed", `GitHub refused to merge pull request #${prNumber}`, merge),
    ]);
