...
import { catalogConventions, resolveConventions } from "./conventions.ts";
...
function usage(): never {
  ...
  coding-tooling conventions resolve [--root <path>] [--config <path>] [--conventions-root <path>] [--registry <path>] [--compact] [--json]
  coding-tooling conventions catalog [--conventions-root <path>] [--write|--check] [--json]`);
  process.exit(2);
}
...
export function main(argv = process.argv.slice(2)): number {
  ...
  if (command === "inspect") result = inspect(root);
    ...
    else if (positional[0] === "catalog")
      result = catalogConventions({
        root,
        conventionsRoot: stringOption(options, "conventions-root"),
