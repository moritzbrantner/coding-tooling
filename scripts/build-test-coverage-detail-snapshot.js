...
export function buildTestCoverageDetailSnapshot({
  files,
  repository,
  revision,
  generatedAt,
  ...
  if (sourceFormat !== "lcov")
    throw new Error("Detailed coverage publication currently supports LCOV only");
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const files = parseCoverageDetail(await readFile(input, "utf8"), format);
...
function usage() {
  throw new Error(
    "Usage: bun scripts/build-test-coverage-detail-snapshot.js --input <path> --format lcov --repository <owner/repository> --revision <sha> --generated-at <iso> --output <path>",
  );
}
