...
- `rust-source-test`: a Rust library source graph has neither inline test evidence nor structural integration-test reachability through the public crate seam.
...
The Rust test expectation follows the same principle: it must not pressure tests to import private implementation modules merely to satisfy a structural detector. The detector builds the ordinary library source graph from `src/lib.rs` through un-attributed `mod name;` declarations whose target resolves unambiguously to a committed `name.rs` or `name/mod.rs`. Inline module scopes are tracked when resolving out-of-line children, so `mod outer { mod child; }` resolves beneath `outer/`. Binary entrypoints, undeclared `.rs` files, attributed or path-overridden modules, and ambiguous module layouts are outside this expectation rather than guessed.

Inline evidence requires both `#[cfg(test)]` and a test attribute in executable Rust code. Line comments, nested block comments, normal/raw strings, and character/byte-character literals are removed conservatively before matching so textual examples cannot manufacture or hide evidence. Integration evidence starts only from Cargo integration-test targets: automatically discovered top-level `tests/*.rs` files when `autotests` is enabled, plus explicit `[[test]]` paths. `autotests` accepts ordinary TOML trailing comments. The detector follows ordinary reachable `mod` declarations from those roots, so an unreferenced nested helper cannot manufacture evidence. Imports inside comments or strings likewise do not count.

When an executable integration-test source imports the library's public crate seam with `use crate_name...` or `extern crate crate_name`, the declared library module graph is structurally test-reachable through the same public boundary the application exposes. That does **not** claim that every behavior, branch, endpoint, or policy in those modules has an assertion; behavioral coverage remains the responsibility of risk-specific tests and other expectations.

A missing Rust finding carries a deterministic Cargo verification command but no default scaffold: creating a TODO, ignored, empty, or private-module-import test merely to silence the finding would not be useful evidence. The verification command currently uses `--locked` only when a package-local `Cargo.lock` is directly proven. An unrelated ancestor lockfile is never assumed to apply to a nested standalone crate; unmodeled workspace-lock relationships therefore fall back to a runnable unlocked command rather than risking a false deterministic failure.
