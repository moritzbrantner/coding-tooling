...
The Rust test expectation follows the same principle: it must not pressure tests to import private implementation modules merely to satisfy a structural detector. The detector first builds the ordinary library source graph from `src/lib.rs` through un-attributed `mod name;` declarations whose target resolves unambiguously to a committed `name.rs` or `name/mod.rs`. Binary entrypoints, undeclared `.rs` files, attributed or path-overridden modules, and ambiguous module layouts are outside this expectation rather than guessed.

Inline evidence requires both `#[cfg(test)]` and a test attribute in executable Rust code; comments and string literals are removed before matching. Integration evidence starts only from Cargo integration-test targets: automatically discovered top-level `tests/*.rs` files when `autotests` is enabled, plus explicit `[[test]]` paths. The detector follows ordinary reachable `mod` declarations from those roots, so an unreferenced nested helper cannot manufacture evidence. Imports inside comments or strings likewise do not count.
...
A missing Rust finding carries a deterministic Cargo verification command but no default scaffold: creating a TODO, ignored, empty, or private-module-import test merely to silence the finding would not be useful evidence. The verification command uses `--locked` only when a package or repository/workspace `Cargo.lock` exists; library repositories without a lockfile remain runnable.
