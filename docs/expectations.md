...
Inline evidence requires both `#[cfg(test)]` and a test attribute in the source file. Integration evidence is intentionally structural: when a `tests/*.rs` file imports the library's public crate seam with `use crate_name...` or `extern crate crate_name`, the declared library module graph is test-reachable through the same public boundary the application exposes. That does **not** claim that every behavior, branch, endpoint, or policy in those modules has an assertion; behavioral coverage remains the responsibility of risk-specific tests and other expectations. A missing Rust finding carries a deterministic `cargo test --locked --manifest-path ...` verification command but no default scaffold: creating a TODO, ignored, empty, or private-module-import test merely to silence the finding would not be useful evidence.
